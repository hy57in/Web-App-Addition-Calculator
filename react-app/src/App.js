...
*/
  ...
  */
    ...
    fetch("http://34.66.13.114:8080/api/insert_nums", {
      ...
      .then((response) => {
        if (response.status === 200) {
          response.json().then((data) => console.log(data["Response"]));
        } else {
          response.json().then((data) => console.log(data["Response"]));
        }
      })
  ...
  onCalculateValues(event) {
    event.preventDefault();
    let operands = {
      firstNum: this.state.firstNumber,
      secondNum: this.state.secondNumber,
      answer: null,
    };

    if (
      isNaN(parseInt(operands.firstNum)) ||
      isNaN(parseInt(operands.secondNum))
    ) {
      this.setState({ answer: "Must enter valid number" });
    } else {
      let calculationAnswer =
        parseInt(operands.firstNum) + parseInt(operands.secondNum);
  ...
  getHistory(event) {
    ...
    */
    ...
    fetch("http://34.66.13.114:8080/api/data", { method: "GET", mode: "cors" })
      .then((response) => {
        if (response.status === 200) {
  ...
  render() {
    return (
      <form method="GET">
        <div>
          <FirstNumber
            onFirstChange={(event) => this.onFirstNumChange(event)}
          />
        </div>
        <div>
          <p>+</p>
        ...
        <div>
          <SecondNumber
            onSecondChange={(event) => this.onSecondNumChange(event)}
          />
        </div>
        <div>
          <Calculate
            onCalculateClick={(event) => this.onCalculateValues(event)}
          />
        </div>
...
function FirstNumber(props) {
  return <input placeholder="0" onChange={props.onFirstChange}></input>;
}

function SecondNumber(props) {
  return <input placeholder="0" onChange={props.onSecondChange}></input>;
}

function Calculate(props) {
  return <button onClick={props.onCalculateClick}>Calculate!</button>;
}

function Answer(props) {
  return <p>{props.answerNumber}</p>;
}

function ShowHistory(props) {
  if (props.data !== null) {
    return <button onClick={props.closeHistory}>Close history</button>;
  } else {
    return <button onClick={props.showHistory}>Show history</button>;
  }
}
  ...
  if (props.data !== null && props.data !== "loading data") {
    return props.data.map((nums) => {
      let operands =
        ...
        " = " +
        nums[2].toString();
      return <div>{operands}</div>;
    });
  } else if (props.data === "loading data") {
    return <div>loading history...</div>;
  } else {
    return <div>History of calculations</div>;
  }
}
